...
async function main() {
  ...
  for (let i = 0; i < allPrayers.length; i++) {
    ...
    const esPath = join(prayerDir, "transcript.es.md");

    if (!existsSync(esPath)) {
      console.log(`  [skip] ${slug} — no ES transcript`);
      continue;
    }

    const transcript = readFileSync(esPath, "utf8");

    for (const speakerKey of SPEAKER_KEYS) {
      const audioPath = join(prayerDir, `narration-${speakerKey}-es.mp3`);
