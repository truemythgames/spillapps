...
# Build list of story IDs missing the elijah narration file.
# (Grace voice is already generated for every story in an earlier pass.)
missing=()
...
if [ "$total" -eq 0 ]; then
  echo "All $total_catalog stories already have the elijah narration file. Nothing to do."
  exit 0
fi
...
for i in "${!missing[@]}"; do
  ...
  if npx tsx scripts/generate.ts --story "$s" --step narration --no-upload \
        > "$log" 2>&1; then
    if [ -f "$CONTENT_DIR/$s/narration-elijah.mp3" ]; then
      size_e=$(du -h "$CONTENT_DIR/$s/narration-elijah.mp3" | cut -f1)
      echo "  OK elijah=$size_e"
      completed=$((completed+1))
    else
      echo "  INCOMPLETE (narration-elijah.mp3 missing) — see $log"
      failed=$((failed+1))
      failed_ids+=("$s")
    fi
  else
