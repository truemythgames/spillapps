import type { StoryWithCover } from "@/stores/app";

export type VerseLang = "en" | "es";
...
const THEMES: {
...
}[] = [
  {
    ...
    sitEn: "Say the verse once out loud before the thing you are avoiding. Not as a pep talk. As a reminder that you are not walking in alone.",
    sitEs: "Di el versículo en voz alta antes de lo que estás evitando. No como arenga. Como recordatorio de que no caminas solo.",
  ...
  },
  {
    ...
    sitEn: "Put one name on the verse. Someone you will text, feed, forgive, or stay with. Love that stays on the page is just ink.",
    sitEs: "Ponle un nombre a este versículo. Alguien a quien vas a escribir, alimentar, perdonar o acompañar. El amor que se queda en la página es solo tinta.",
  ...
  },
  {
    ...
    sitEn: "When the next spiral starts, stop at one sentence. Read the verse. Breathe once. Then do the next ordinary thing in front of you.",
    sitEs: "Cuando empiece el siguiente remolino, párate en una frase. Lee el versículo. Respira una vez. Luego haz lo ordinario que tienes delante.",
  ...
  },
  {
    ...
    sitEn: "Do not wait to feel brave. Take the smallest honest step this verse is pointing at — the call, the apology, the door you keep walking past.",
    sitEs: "No esperes a sentirte valiente. Da el paso honesto más pequeño al que apunta este versículo: la llamada, la disculpa, la puerta que evitas.",
  ...
  },
  {
    ...
    sitEn: "Name what you keep dragging. Then leave it in the verse — not as denial, as a decision that you are done being the judge of your own case.",
    sitEs: "Nombra lo que sigues arrastrando. Luego déjalo en el versículo: no como negación, sino como decisión de dejar de ser el juez de tu propio caso.",
  ...
  },
  {
    ...
    sitEn: "Do not fill the wait with a worse story. Hold the verse the way you would hold a letter from someone who always shows.",
    sitEs: "No llenes la espera con una historia peor. Sostén el versículo como sostendrías una carta de alguien que siempre llega.",
  ...
  },
  {
    ...
    sitEn: "Take the verse into the dim room — the decision, the mood, the conversation you keep postponing. Ask for enough light for the next step, not the whole map.",
    sitEs: "Lleva el versículo al cuarto oscuro: la decisión, el ánimo, la conversación que aplazas. Pide luz para el siguiente paso, no para todo el mapa.",
  ...
  },
  {
    ...
    sitEn: "Stop polishing the prayer. Tell Him the weak part first. This verse says He already knows the weight, and He is not leaving the room.",
    sitEs: "Deja de pulir la oración. Dile primero la parte débil. Este versículo dice que Él ya conoce el peso, y no se va de la habitación.",
...
const FALLBACK = {
  ...
  sitEn: "Read it twice. Once for the mind, once for the day you are actually in. Then take one sentence with you into the next room.",
  sitEs: "Léelo dos veces. Una para la mente, otra para el día que de verdad estás viviendo. Luego llévate una frase a la siguiente habitación.",
...
export function canonBook(raw: string): string {
  const key = stripMarks(raw).toLowerCase().replace(/\s+/g, " ").trim();
  return BOOK_ALIASES[key] ?? raw.trim();
}

export function parseBibleRef(ref: string): { book: string; chapters: number[] } | null {
  const trimmed = ref.replace(/\s+/g, " ").trim();
  if (!trimmed) return null;

  const song = trimmed.match(/^(Song of (?:Solomon|Songs)|Cantar de los Cantares|Cantares)\s+(.+)$/i);
  const numbered = trimmed.match(/^(\d\s+[A-Za-zÀ-ÿ]+)\s+(.+)$/i);
  const named = trimmed.match(/^([A-Za-zÀ-ÿ]+)\s+(.+)$/i);
  const match = song ?? numbered ?? named;
  if (!match) return null;

  const book = canonBook(match[1]);
  const rest = match[2];
  const chapters = new Set<number>();

  for (const part of rest.split(/[,;]/)) {
    const range = part.match(/(\d+)\s*[-–]\s*(\d+)/);
    if (range && !part.includes(":")) {
      const start = Number(range[1]);
      const end = Number(range[2]);
      for (let n = start; n <= end && n <= start + 20; n++) chapters.add(n);
      continue;
    }
    const chapter = part.match(/(\d+)/);
    if (chapter) chapters.add(Number(chapter[1]));
  }
...
export function getVerseInsight(
  enVerse: { text: string; ref: string },
  localVerse: { text: string; ref: string },
  locale: VerseLang,
): VerseInsight {
  const parsed = parseBibleRef(enVerse.ref) ?? parseBibleRef(localVerse.ref);
  const book = parsed?.book ?? canonBook(enVerse.ref);
  const ctx =
    BOOK_CONTEXT[book]?.[locale] ??
    BOOK_CONTEXT[book]?.en ??
    BOOK_CONTEXT.Psalms?.[locale] ??
    "";
  const theme = themeFor(`${enVerse.text} ${localVerse.text}`);
  const reflection = theme
    ? locale === "es" ? theme.es : theme.en
    : locale === "es" ? FALLBACK.es : FALLBACK.en;
  const practice = theme
    ? locale === "es" ? theme.sitEs : theme.sitEn
    : locale === "es" ? FALLBACK.sitEs : FALLBACK.sitEn;
  ...
  return {
    book,
    testament: NT_BOOKS.has(book) ? "new" : "old",
    context: ctx,
    reflection,
    practice,
    question,
    relatedSlug: RELATED[book]?.slug ?? RELATED.Psalms?.slug,
  };
}
...
export function storiesForVerse(
  stories: StoryWithCover[],
  enRef: string,
  localRef?: string,
): StoryWithCover[] {
  const verse = parseBibleRef(enRef) ?? parseBibleRef(localRef ?? "");
  if (!verse || stories.length === 0) return [];

  const chapterHits: StoryWithCover[] = [];
  const sameBook: StoryWithCover[] = [];

  for (const story of stories) {
    const parsed = parseBibleRef(story.bibleRef);
    if (!parsed || canonBook(parsed.book) !== verse.book) continue;
    if (parsed.chapters.some((c) => verse.chapters.includes(c))) {
      chapterHits.push(story);
    } else {
      sameBook.push(story);
    }
  }

  const out: StoryWithCover[] = [];
  const seen = new Set<string>();
  const push = (story?: StoryWithCover | null) => {
    if (!story) return;
  ...
  chapterHits.forEach(push);
  sameBook.forEach(push);

  if (out.length < 3) {
    const slug = RELATED[verse.book]?.slug ?? RELATED.Psalms?.slug;
    push(stories.find((s) => s.id === slug || s.apiId === slug));
  }
