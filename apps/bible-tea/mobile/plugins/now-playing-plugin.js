...
module.exports = function withNowPlaying(config) {
  ...
  config = withDangerousMod(config, [
    ...
    (config) => {
      ...
      fs.writeFileSync(path.join(pkgDir, "NowPlayingBridgeModule.kt"), ANDROID_MODULE_KT);
      fs.writeFileSync(path.join(pkgDir, "PlaybackService.kt"), ANDROID_SERVICE_KT);
      fs.writeFileSync(path.join(pkgDir, "NowPlayingBridgePackage.kt"), ANDROID_PACKAGE_KT);
      return config;
    },
  ]);
