...
`android.package` in `app.json` (currently `app.truecrimetea`).
...
- iOS: `app.truecrimetea`
- Android: `app.truecrimetea`
- Scheme: `truecrimetea`
- App slug: `truecrimetea`

## Content

`apps/true-crime-tea/content/` was reset to empty catalogs. Generate with:
...
Both honor `CONTENT_APP_ID=true-crime-tea` and `R2_APP_PREFIX=true-crime-tea`.
